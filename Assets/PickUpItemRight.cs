...
public class PickUpItemRight : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(1) && pickedUp)
        {
            chargeTime += Time.deltaTime;
            chargeTime = Mathf.Min(chargeTime, maxChargeTime);
        }
        if (Input.GetMouseButtonUp(1) && pickedUp)
        {
    ...
    public void PickUp()
    {
        ...
        if (Physics.Raycast(playerCam.transform.position, playerCam.transform.forward, out hit, distance))
        {
            if (hit.transform.tag == "Item")
            {
                ...
                canPickUp = true;
                Debug.Log("прав корутина началась");
    ...
    public void Throw(float charge)
    {
        ...
        if (currentItem != null)
        {
            ...
            Debug.Log("прав бросок с силой " + actualForce);
            canPickUp = false;
            pickedUp = false;
            currentItem = null;
        }
